...
public class AccumulateTests
{
    ...
    {
        Assert.Equal(Array.Empty<int>(),
            Array.Empty<int>().Accumulate(x => x * x));
    }
    ...
    {
        Assert.Equal([ 1, 4, 9 ],
            new[] { 1, 2, 3 }.Accumulate(x => x * x));
    }

    [Fact]
    public void Accumulate_upcases()
    {
        Assert.Equal(new List<string> { "HELLO", "WORLD" },
            new List<string> { "hello", "world" }.Accumulate(x => x.ToUpper()));
    }
    ...
    public void Accumulate_reversed_strings()
    {
        Assert.Equal("eht kciuq nworb xof cte".Split(' '),
            "the quick brown fox etc".Split(' ').Accumulate(Reverse));
    }
