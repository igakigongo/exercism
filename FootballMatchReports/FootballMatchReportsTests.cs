...
public class FootballMatchReportsTests
{
    ...
    public void AnalyzeOnField_throws_unknown_shirt_number()
    {
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            PlayAnalyzer.AnalyzeOnField(1729));
    }

    [Fact]
    public void AnalyzeOffField_number()
    {
        Assert.Equal("There are 4200 supporters at the match.",
            PlayAnalyzer.AnalyzeOffField(4200));
    }
    ...
    public void AnalyzeOffField_throws_unknown_type()
    {
        Assert.Throws<ArgumentException>(() =>
            PlayAnalyzer.AnalyzeOffField(90.0f));
    }

    [Fact]
    public void AnalyzeOffField_text()
    {
        Assert.Equal("They think it's all over!",
    ...
    {
        Assert.Equal("An incident happened.",
            PlayAnalyzer.AnalyzeOffField(new Incident()));
    }

    [Fact]
    public void AnalyzeOffField_foul()
    {
        Assert.Equal("The referee deemed a foul.",
    ...
    {
        Assert.Equal("Oh no! Player 3 is injured. Medics are on the field.",
    ...
    {
        Assert.Equal("David Moyes",
            PlayAnalyzer.AnalyzeOffField(new Manager("David Moyes", null)));
    }

    [Fact]
    public void AnalyzeOffField_manager_with_club()
    {
        Assert.Equal("Thomas Tuchel (Chelsea)",
            PlayAnalyzer.AnalyzeOffField(
                new Manager("Thomas Tuchel", "Chelsea")));
    }
}
